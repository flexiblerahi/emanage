    ...
    @if(isset($parent))
        <div class="col-2">
            <div class="form-group">
                ...
                <br>
                @if(isset($end) && isset($selected))
                    <button type="button" class="btn btn-danger @if($end != $selected) d-none @endif btn-sm removebtn" id="remove-{{$parent->id}}" data-parent="{{$parent->id}}" data-parentSelect={{$previous}} ><i class="text-white fa fa-minus" aria-hidden="true"></i></button>
                @endif
            </div>
        </div>
    @else
